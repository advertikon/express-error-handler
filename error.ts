...
export async function FetchError (response: Response): Promise<VError> {
  ...
  return new VError(
    ...
    'Error querying %s: %s',
    response.url,
    validationMessage || response.statusText
  );
}
...
export function CreateErrorByCode (code: number, message: string, error?: Error): VError {
  return new VError(
    {
      name: getErrorByCode(code),
      info: { code },
      cause: error,
    },
    message
...
export function CreateUnprocessedEntityError (
  message: string,
  info = {},
  cause?: Error
): VError {
